...
function updateAuthUI() {
  ...
  if (isLoggedIn()) {
    navCta.innerHTML = `
      <span class="user-greeting" style="color:var(--gold);font-family:'JetBrains Mono',monospace;font-size:0.8rem;">
        <i class="fa-solid fa-circle-user"></i> ${currentUser.fullName}
      </span>
      <button class="btn-ghost" onclick="logout()"><i class="fa-solid fa-right-from-bracket"></i> Logout</button>
      <button class="btn-primary" onclick="openInterviewSetup()"><i class="fa-solid fa-rocket"></i> Interview</button>
    `;
  } else {
    navCta.innerHTML = `
      <button class="btn-ghost" onclick="openModal('loginModal')"><i class="fa-regular fa-user"></i> Sign In</button>
      <button class="btn-primary" onclick="openModal('signupModal')"><i class="fa-solid fa-rocket"></i> Start Free</button>
    `;
  }
}
...
async function handleSignup() {
  ...
  const result = await apiCall('/auth/signup', 'POST', { fullName, email, password });
  ...
  if (result.ok) {
    ...
    updateAuthUI();
    showToast('🎉 Welcome to InterviewForge, ' + currentUser.fullName + '!');
  } else {
    showToast('❌ ' + (result.data.message || 'Signup failed'));
  }
}
...
function openInterviewSetup() {
  if (!isLoggedIn()) {
    showToast('🔒 Please sign in to start an interview');
...
/* ===== INTERVIEW QUESTIONS (client-side fallback) ===== */
...
let loadedQuestions = []; // questions from API
let currentDomain = '';

/* ===== START INTERVIEW (from domain cards) ===== */
async function startInterview(domain) {
  ...
  const difficulty = diffSlider ? ['EASY', 'MEDIUM', 'HARD'][parseInt(diffSlider.value) - 1] : 'MEDIUM';

  const result = await apiCall('/interviews/start', 'POST', {
    domain: domain,
    role: domain,
  ...
  const qResult = await apiCall('/questions?domain=' + encodeURIComponent(domain));
  if (qResult.ok && qResult.data.data && qResult.data.data.length > 0) {
    loadedQuestions = qResult.data.data;
  } else {
    // Fallback to client-side questions
    loadedQuestions = (interviewQuestions[domain] || interviewQuestions['default']).map((q, i) => ({
      id: i + 1,
      questionText: q,
      domain: domain,
      difficulty: 'MEDIUM'
...
/* ===== LAUNCH INTERVIEW (from setup modal) ===== */
async function launchInterview() {
  const role = document.getElementById('roleSelect').value;
  const diffVal = document.getElementById('diffSlider').value;
  const difficulty = ['EASY', 'MEDIUM', 'HARD'][parseInt(diffVal) - 1];

  // Get selected chips
  const companyChips = document.querySelectorAll('#companyChips .chip.active');
  const companyType = Array.from(companyChips).map(c => c.textContent.trim()).join(',') || 'FAANG';
  ...
  // Load questions
  const qResult = await apiCall('/questions?domain=' + encodeURIComponent(role) + '&difficulty=' + difficulty);
  if (qResult.ok && qResult.data.data && qResult.data.data.length > 0) {
    loadedQuestions = qResult.data.data;
  } else {
    loadedQuestions = (interviewQuestions[role] || interviewQuestions['default']).map((q, i) => ({
      id: i + 1,
      questionText: q,
      domain: role,
      difficulty: difficulty
...
function loadQuestion(domain) {
  const questions = loadedQuestions.length > 0 ? loadedQuestions : 
    (interviewQuestions[domain] || interviewQuestions['default']).map((q, i) => ({
      id: i + 1, questionText: q
    }));
  ...
  setTimeout(() => {
    display.innerHTML = `
      <div style="text-align:left;">
        ...
        <p style="font-size:0.95rem; line-height:1.7;">${qText}</p>
      </div>
    `;
    animateLiveMetrics();
  }, 1200);
...
async function submitAnswer() {
  ...
  const questionId = currentQ.id || currentQuestionIndex + 1;

  // Submit to backend
  if (currentSessionId) {
    const result = await apiCall(`/interviews/${currentSessionId}/answer`, 'POST', {
    ...
    if (result.ok && result.data.data) {
      ...
      // Update live metrics with actual scores
      const confBar = document.getElementById('confBar');
      const clarBar = document.getElementById('clarBar');
      const relBar = document.getElementById('relBar');
      if (confBar) confBar.style.width = scores.confidenceScore + '%';
      if (clarBar) clarBar.style.width = scores.clarityScore + '%';
      if (relBar) relBar.style.width = scores.relevanceScore + '%';
    }
  }
  ...
  loadQuestion(currentDomain || document.getElementById('roomRole').textContent.replace(' Interview', ''));
}
...
async function endInterview() {
  ...
  let tech, comm, prob, conf, overall, aiFeedback;

  // Try to get real scores from backend
  if (currentSessionId) {
    const result = await apiCall(`/interviews/${currentSessionId}/end`, 'POST');
    if (result.ok && result.data.data) {
  ...
  // Fallback to random scores if backend unavailable
  if (!tech) {
    tech = Math.floor(65 + Math.random() * 30);
    comm = Math.floor(60 + Math.random() * 35);
    prob = Math.floor(55 + Math.random() * 40);
    conf = Math.floor(70 + Math.random() * 25);
    overall = Math.floor((tech + comm + prob + conf) / 4);
  }
...
function typeWriterFeedback(feedback) {
